...
def draw_heatmap(slideID, attention_dir, save_dir):
    ...
    thumb_x10 = Image.open(f"thumb/{slideID}_thumb.tif")
    thumb_x20 = Image.open(f"thumb/{slideID}_thumb.tif")
    w, h = thumb_x10.size
    w_num = w // t_size
    h_num = h // t_size
    draw_x10 = ImageDraw.Draw(thumb_x10)
    draw_x20 = ImageDraw.Draw(thumb_x20)
    makedir(save_dir)

    # read attention weights for slide
    attention_file = f"{attention_dir}/{slideID}_norm.csv"
    with open(attention_file, "r") as f:
        reader = csv.reader(f)
        # header = next(reader)  # when header should be skipped
        for row in reader:
            block = row[0].split("_")[2].split(".")[0]  # patch ID
            mag = row[0].split("_")[0]
            if "r" in block:
    ...
    thumb_x10.save(f"{save_dir}/{slideID}_x10_map.tif")
    thumb_x20.save(f"{save_dir}/{slideID}_x20_map.tif")
