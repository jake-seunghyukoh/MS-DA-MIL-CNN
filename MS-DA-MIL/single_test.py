...
if __name__ == "__main__":
    ...
    train_DLBCL, train_nonDLBCL, test_DLBCL, test_nonDLBCL = ds.slide_split(
    ...
    makedir("test_result")
    test_result = (
        f"test_result/{mag}_train-{train_slide}_test-{test_slide}_DArate-{DArate}.csv"
    )
    # validation
    f = open(test_result, "w")
    f.close()
    test_data = utils.make_testdata(test_DLBCL, 1, mag) + utils.make_testdata(
        test_nonDLBCL, 0, mag
    )
