...
def data_load(data):
    ...
    tensor_list = [
        torch.Tensor(cv2.imread(patch_path).transpose((2, 1, 0)))
        for patch_path in data[0]
    ]
...
def data_load_multi(data):
    tensor_list_x10 = [
        torch.Tensor(cv2.imread(img_path).transpose((2, 1, 0))) for img_path in data[0]
    ]
    tensor_list_x20 = [
        torch.Tensor(cv2.imread(img_path).transpose((2, 1, 0))) for img_path in data[1]
    ]
...
def test_data_load_multi(data):
    ...
    for image_path in data[0]:
        ...
        img = cv2.imread(image_path).transpose(
            (2, 1, 0)
        )  # read image shape=(224,224,3)
        img_tensor = torch.Tensor(img)
